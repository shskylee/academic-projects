...
// Monte carlo simulation for SIS model on hierarhical modular networks
template<typename T>
struct Node{
    T data;
    Node<T> *next;
...
int main(int argc, char* argv[]){
    ...
    for(int i=0;i<num;i++){
       for(int j=0;j<cn;j++){
           for(int k=j+1;k<cn;k++){
               adjacency[i*cn+j][edges_count[i*cn+j]]=i*cn+k;
               adjacency[i*cn+k][edges_count[i*cn+k]]=i*cn+j;
               edges_count[i*cn+j]++;
               edges_count[i*cn+k]++;
    }}}
    ...
    for (int i=1; i<g; i++) {
        int m=pow(2,g-1-i);
        for (int j=1; j<m+1; j++) {
            int k=pow(2,i-1)*(4*j-3);
            int l=pow(2,i-1)*(4*j-1);
            e1=k*cn+rand()%cn;
            e2=l*cn+rand()%cn;
            adjacency[e1][edges_count[e1]]=e2;
            adjacency[e2][edges_count[e2]]=e1;
    ...
    if(used_time>3600.0){
     ...
     string filename1;
     string filename2;
     filename1=(string)"sis_HN3_g"+argv[1]+"cn"+argv[2]+"l"+argv[3]+"run"+argv[4]+".txt";
     filename2=(string)"time_cost_HN3_g"+argv[1]+"cn"+argv[2]+"l"+argv[3]+"run"+argv[4]+".txt";
